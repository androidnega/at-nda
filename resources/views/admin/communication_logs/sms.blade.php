...
<form method="post" action="{{ route('dashboard.communication-logs.sms.purge', request()->query()) }}" id="sms-delete-filtered-form" onsubmit="return confirm('Delete all filtered SMS logs? This cannot be undone.');">
    @csrf
    @method('DELETE')
</form>

<form method="post" action="{{ route('dashboard.communication-logs.sms.bulk') }}" id="sms-bulk-form">
    @csrf
    ...
    <div class="mb-6 flex flex-wrap gap-2 items-center">
        <a href="{{ route('dashboard.communication-logs.sms.export-csv', request()->query()) }}" class="px-4 py-2 rounded-lg text-sm border border-gray-200 text-gray-700 hover:bg-gray-50">Export filtered CSV</a>
        <a href="{{ route('dashboard.communication-logs.sms.download-zip', request()->query()) }}" class="px-4 py-2 rounded-lg text-sm border border-gray-200 text-gray-700 hover:bg-gray-50">Download filtered ZIP</a>
        <button type="submit" form="sms-delete-filtered-form" class="px-4 py-2 rounded-lg text-sm border border-rose-200 text-rose-700 hover:bg-rose-50">Delete filtered</button>
        ...
        <button type="submit" onclick="return confirmSmsBulkAction();" class="px-4 py-2 rounded-lg text-sm border border-gray-200 text-gray-700 hover:bg-gray-50">Apply to selected</button>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase tracking-wide">
                <tr>
                    <th class="px-4 py-3 w-10"><input type="checkbox" id="sms-select-all" class="rounded border-gray-300"></th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($logs as $row)
                    <tr class="hover:bg-gray-50/80">
                        <td class="px-4 py-3"><input type="checkbox" name="selected_ids[]" value="{{ $row->id }}" class="sms-row-check rounded border-gray-300"></td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('dashboard.communication-logs.sms.export-csv', array_merge(request()->query(), ['id' => $row->id])) }}" class="px-2.5 py-1.5 rounded-md text-xs border border-gray-200 text-gray-700 hover:bg-gray-50">Export</a>
                                <form method="post" action="{{ route('dashboard.communication-logs.sms.delete-single', $row) }}" onsubmit="return confirm('Delete this message?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-4 py-12 text-center text-gray-500">No SMS logs yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
function confirmSmsBulkAction() {
    const form = document.getElementById('sms-bulk-form');
    const action = form?.querySelector('select[name="action"]')?.value;
    if (action === 'delete') {
        return confirm('Delete selected SMS logs? This cannot be undone.');
    }
    return true;
}
</script>

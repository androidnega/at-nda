            ...
            @else
                <form method="POST" action="{{ route('dashboard.timetable.store') }}" class="space-y-3">
                    ...
                    <div>
                        ...
                        <select name="venue_id" class="w-full border-2 border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/25 focus:border-primary">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1">Venue note (optional)</label>
                        <input type="text" name="venue" value="{{ old('venue') }}" maxlength="255" placeholder="e.g. Hall B" class="w-full border-2 border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/25 focus:border-primary">
                        @error('venue') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>

                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-white px-4 py-2.5 text-sm font-semibold hover:bg-primary/90">
                                ...
                                <div class="px-4 pb-4 bg-gray-50/60">
                                    <form method="POST" action="{{ route('dashboard.timetable.update', $entry) }}" class="grid grid-cols-1 sm:grid-cols-6 gap-3 pt-3">
                                        ...
                                        <div class="sm:col-span-3">
                                            ...
                                            <select name="venue_id" class="w-full border-2 border-gray-200 rounded-lg px-2.5 py-2 text-xs focus:ring-2 focus:ring-primary/25 focus:border-primary">
                                                <option value="">— Choose venue —</option>
                                                @foreach($availableVenues as $venue)
                                                    <option value="{{ $venue->id }}" @selected($venue->id === $entry->venue_id)>{{ $venue->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="sm:col-span-3">
                                            <label class="block text-[11px] font-semibold text-gray-700 mb-1">Venue note</label>
                                            <input type="text" name="venue" value="{{ $entry->venue }}" maxlength="255" class="w-full border-2 border-gray-200 rounded-lg px-2.5 py-2 text-xs focus:ring-2 focus:ring-primary/25 focus:border-primary">
                                        </div>

...
<div class="w-full {{ $pl === 'courserep' ? 'max-w-6xl mx-auto' : 'max-w-lg sm:max-w-xl' }}">
    <div class="rounded-2xl bg-white border border-slate-200/80 shadow-sm p-5 sm:p-7">
        ...
        <div class="mt-4 flex justify-center">
            <img src="{{ $student->profileImageUrl() }}" alt="" class="h-24 w-24 rounded-full object-cover border-2 border-slate-100 shadow-sm">
        </div>
        @endif
        ...
        <form method="POST" action="{{ route('student.profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-4">
            ...
            <div>
                ...
                <select id="faculty_id" name="faculty_id" required class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-sky-500/30 focus:border-sky-500">
            ...
            <div>
                ...
                <select id="department_id" name="department_id" required class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-sky-500/30 focus:border-sky-500">

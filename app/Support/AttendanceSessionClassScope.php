...
final class AttendanceSessionClassScope
{
     ...
     * passes in `$student->attendances()->whereIn(...)->activeWeeksOnly()`
     * which is still a HasMany — Eloquent relations forward where / whereHas
     * / whereRaw via __call, so the body works on both, but PHP's strict
     * scalar type-hint used to reject the Relation path and bubble a hard
     * TypeError on production (rep /dashboard/students/{id} 500).
     * Same widening precedent as {@see self::applyForClass()}.
     *
